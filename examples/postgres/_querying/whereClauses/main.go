...
func main() {
	...
	//db.Debug().Where("username like ? and first_name = ?", "%mac%", "Tricia").
	//	Find(&users)

	//db.Debug().Where("created_at BETWEEN ? and ?", time.Now().Add(-30*24*time.Hour),
	//	time.Now()).Find(&users)

	//db.Debug().Not("username = ?", "adent").Find(&users)

	db.Debug().Where("username = ?", "adent").Or("username = ?", "fprefect").Find(&users)
